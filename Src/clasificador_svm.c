 ...
 * @brief Implementation of functions to initialize and predict using RBF SVMs.
 */
...
static arm_svm_rbf_instance_f32 svm_blue_ctx;
static arm_svm_rbf_instance_f32 svm_green_ctx;
static arm_svm_rbf_instance_f32 svm_red_ctx;

// Array to easily iterate SVM contexts during prediction
static arm_svm_rbf_instance_f32* svm_ctx_array[NB_CLASSIFIERS];
...
{
    // blue vs rest
    arm_svm_rbf_init_f32(&svm_blue_ctx,
                         NB_SV_BLUE_VS_REST,
                         VECTOR_DIMENSION,
                         ...
                         GAMMA_BLUE_VS_REST);
    // green vs rest
    arm_svm_rbf_init_f32(&svm_green_ctx,
                         NB_SV_GREEN_VS_REST,
                         VECTOR_DIMENSION,
                         ...
                         GAMMA_GREEN_VS_REST);    
    // red vs rest
    arm_svm_rbf_init_f32(&svm_red_ctx,
                         NB_SV_RED_VS_REST,
                         VECTOR_DIMENSION,
...
void clasificador_svm_predict(float32_t *feature_vector, uint32_t feature_vector_size, int32_t *label)
{
    ...
    *label = -1;
    ...
    {
        arm_svm_rbf_predict_f32(svm_ctx_array[i], feature_vector, label);
